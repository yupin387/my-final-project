...
public class Run {
	...
	public static void main(String[] args) {
		...
		try {
			...
			ceremonyItemRepo.save(
					new CeremonyItem(c1, houseFlower, 1)
			);
			...
			// ✅ เพิ่ม: พานพุ่มดอกไม้สดถวายพระ (ของเฉพาะงานทำบุญบ้าน)
			ceremonyItemRepo.save(
					new CeremonyItem(c2, houseFlower, 1)
			);
			...
			// ✅ เพิ่ม: พานพุ่มดอกไม้สดถวายพระ (ของเฉพาะงานทำบุญบ้าน)
			ceremonyItemRepo.save(
					new CeremonyItem(c3, houseFlower, 1)
			);

			// ✅ เพิ่ม: ของแถมระดับพรีเมียม (ให้เท่ากับพรีเมียมออฟฟิศ c9)
			ceremonyItemRepo.save(
					new CeremonyItem(c3, photographer, 1)
			);

			ceremonyItemRepo.save(
					new CeremonyItem(c3, premiumFlower, 1)
			);
			...
			// ✅ เพิ่ม: ของแถมระดับพรีเมียม (ให้เท่ากับพรีเมียมออฟฟิศ c9)
			ceremonyItemRepo.save(
					new CeremonyItem(c6, photographer, 1)
			);

			ceremonyItemRepo.save(
					new CeremonyItem(c6, premiumFlower, 1)
			);
			...
			List<Ceremony> standardCeremonies = Arrays.asList(c1, c4, c7);
			for (Ceremony c : standardCeremonies) {
				ceremonyItemRepo.save(
						new CeremonyItem(c, sanghaStandard, 5)
				);
			}

			// แพ็กเกจอิ่มบุญ (7 พระ) -> สังฆทาน 7 ชุด
			List<Ceremony> fullMeritCeremonies = Arrays.asList(c2, c5, c8);
			for (Ceremony c : fullMeritCeremonies) {
				ceremonyItemRepo.save(
						new CeremonyItem(c, sanghaStandard, 7)
				);
			}

			// แพ็กเกจพรีเมียม (9 พระ) -> สังฆทาน 9 ชุด
			List<Ceremony> premiumCeremonies = Arrays.asList(c3, c6, c9);
			for (Ceremony c : premiumCeremonies) {
				ceremonyItemRepo.save(
						new CeremonyItem(c, sanghaStandard, 9)
				);
			}
