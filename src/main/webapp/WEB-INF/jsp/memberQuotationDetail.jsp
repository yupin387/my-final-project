...
<link
	href="https://fonts.googleapis.com/css2?family=Sarabun:wght@300;400;600;700&family=Noto+Serif+Thai:wght@400;600;700&display=swap"
	rel="stylesheet">
<link rel="stylesheet"
...
<body>
	...
	<nav class="navbar-custom">
		...
		<div class="navbar-center">
			...
			<a href="${pageContext.request.contextPath}/myBookings"
				class="nav-link-item">รายการจอง</a> <a
	...
	<div class="page-wrapper">
		...
		<div class="flash-banner flash-banner-success no-print"
			id="ajaxConfirmBanner" style="display: none;">✓
			ยืนยันข้อมูลใบเสนอราคาเรียบร้อยแล้ว</div>
				...
				<div class="meta-box-left">
					<table class="layout-table">
						<tr>
							...
							<td class="value">คุณ ${sessionScope.user.memberFirstName}
								${sessionScope.user.memberLastName}</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td class="value"><fmt:formatDate
									value="${q.bookingForm.eventDate}" pattern="dd/MM/yyyy" /> เวลา
								${q.bookingForm.eventTime} น.</td>
						</tr>
						<tr>
				...
				<div class="meta-box-right">
					<table class="layout-table bordered">
						...
						<tr>
							...
							<td class="meta-value"><fmt:formatDate
									value="${q.quotationDate}" pattern="dd/MM/yyyy" /></td>
						</tr>
						<tr>
							<td class="meta-label">สถานะ:</td>
							<td class="meta-value"><span
						...
						<tr>
							<td class="meta-label" style="white-space: nowrap;">หัวหน้างาน:</td>
							<td class="meta-value"><c:choose>
				...
				<tbody>
					...
					<c:if test="${not empty packageIncludedItems}">
						...
						<c:forEach var="subItem" items="${packageIncludedItems}">
							...
							<c:if
								test="${(not empty subItem.itemDetail && fn:contains(subItem.itemDetail,'ต่อรูป')) || fn:contains(subItem.itemName,'ต่อรูป') || fn:contains(subItem.itemName,'พระสงฆ์')}">
								<c:set var="subItemQty"
									value="${not empty monkCount ? monkCount : 1}" />
							</c:if>
							<tr class="package-included-row">
								<td class="text-center"></td>
			...
			<div class="summary-flex-row">
				<c:if test="${q.quotationStatus != 'Confirmed'}">
					<div class="member-note-section no-print">
						...
						<textarea id="memberNoteInput"
							placeholder="ระบุความต้องการเพิ่มเติมที่นี่..."></textarea>
					</div>
				</c:if>
						...
						<c:forEach var="d" items="${details}">
							<c:if test="${d.item != null}">
								<c:choose>
									...
									<c:when
										test="${!isCustomRequest && d.item.itemName == packageName}">
									</c:when>
									<c:otherwise>
										...
										<c:if
											test="${d.item.itemName == 'บริการประสานงานนิมนต์พระ' && isMonkSelfInvite}">
											<c:set var="itemVal" value="0" />
											<c:set var="isFreeInTotal" value="true" />
										</c:if>
										...
										<c:choose>
											<c:when test="${isCustomRequest}">
												...
												<c:choose>
													<c:when
														test="${d.item.itemType.itemTypeName.contains('อุปกรณ์เสริม')}">
						...
						<table class="totals-table">
							...
							<tr class="grand-total-row">
								...
								<td class="total-amount">฿ <fmt:formatNumber
										value="${sumPackage + sumExtra - (isMonkSelfInvite && !isCustomRequest ? 1500 : 0)}"
										minFractionDigits="2" />
								</td>
							</tr>
						</table>
				...
				<c:choose>
					<c:when test="${q.quotationStatus != 'Confirmed'}">
						<div class="btn-flex-group">
							<button type="button" class="btn-revise-submit"
								onclick="packAndSubmitReviseForm()">↩
								ส่งข้อมูลแจ้งขอแก้ไขรายการ</button>
							<button type="button" class="btn-confirm-custom"
								onclick="showConfirmModal()">✓ ยืนยันรายการจองนี้</button>
						</div>
					</c:when>
					<c:otherwise>
						<div class="lock-message">
							<div class="lock-title">ขอบคุณสำหรับการยืนยันการจอง</div>
							<p class="lock-desc">ทางเราได้รับข้อมูลของท่านแล้ว
								และกำลังจัดเตรียมอุปกรณ์พร้อมเจ้าหน้าที่เพื่อให้บริการท่านอย่างดีที่สุด</p>
						</div>
					</c:otherwise>
				</c:choose>
			</div>
		...
		<div style="text-align: center; margin-bottom: 30px;" class="no-print">
			<a href="${pageContext.request.contextPath}/home"
				class="btn-back-bottom">← กลับสู่หน้าหลัก</a>
		</div>
	</div>

	<form id="cleanSubmitForm"
		action="${pageContext.request.contextPath}/member/quotation/revise-all"
	...
	<div id="confirmModal" class="custom-modal">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<p class="text-danger">* เมื่อยืนยันแล้ว
					ระบบจะเริ่มดำเนินการจัดเตรียมงานทันทีและจะไม่สามารถแก้ไขรายการได้</p>
			</div>
			<div class="modal-footer">
				<button type="button" class="btn-secondary"
					onclick="closeConfirmModal()">ยกเลิก</button>
				<form id="confirmQuotationForm"
					action="${pageContext.request.contextPath}/member/quotation/confirm"
					method="post" style="margin: 0;">
					<input type="hidden" name="quotationId" value="${q.quotationId}">
					<button type="submit" class="btn-confirm-final"
						style="background-color: #28a745; color: #ffffff;">ยืนยันอนุมัติ</button>
				</form>
			</div>
		</div>
	</div>
